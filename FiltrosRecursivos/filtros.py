...
def recursive_image(image, num_steps, grayscale=False):
    ...
    for y in range(0, height, step_size):
        for x in range(0, width, step_size):
            ...
            # Si es gris, ajustamos el tono
            if grayscale:
                avg_gray = np.mean(avg_color)
                avg_color = np.array([avg_gray] * 3)

            # Redimensionamos la imagen y ajustamos el tono
            ...
            # Redimensionamos adjusted_img para que coincida exactamente con el tamaño de la región
            adjusted_img = cv2.resize(adjusted_img, (x_end-x, y_end-y), interpolation=cv2.INTER_LANCZOS4)

            # Asignamos la imagen ajustada a la región correspondiente
